...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `RegexSyntax.swiftmodule/`
...
//      `import RegexSyntax` above would fail at compile time.
//
//   2. The static archive `libRegexSyntax.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "RegexSyntax" }` and
//      `flattenPackage = "io.github.kotlinmania.regexsyntax"` configuration in
...
//      via the `RegexSyntaxLibrary` product.
//
...
final class RegexSyntaxExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "RegexSyntax swift module imported cleanly")
    }
}
